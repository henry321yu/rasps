...
def get_img_socket(ip):
    """取得或建立對目標 IP 的 TCP 連線"""
    if ip not in tcp_img_sockets:
        try:
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            s.settimeout(2.0) # 設定連線超時，避免卡死
            s.connect((ip, IMAGE_PORT))
            s.settimeout(None) # 連線成功後恢復阻塞模式
            tcp_img_sockets[ip] = s
        except Exception as e:
            return None
    return tcp_img_sockets[ip]

# ... 中間保留 ...

# [修改] 影像處理與發送執行緒 (改用 TCP 並移除 60KB 限制)
def send_camera():
    ...
    while True:
        ...
        if success:
            ...
            # [關鍵] 打包：4 bytes 長度標頭 + 實際圖片資料
            size_header = len(jpeg_bytes).to_bytes(4, byteorder='big')
            packet = size_header + jpeg_bytes
            
            for ip, _ in REMOTE_PC_LIST:
                if online_status[ip]:
                    sock = get_img_socket(ip)
                    if sock:
                        try:
                            # 透過 TCP 確保完整傳送
                            sock.sendall(packet)
                            ...
                            # 如果傳送失敗（例如對方斷線），關閉並清除 Socket，下次迴圈會重新嘗試連線
                            sock.close()
                            if ip in tcp_img_sockets:
                                del tcp_img_sockets[ip]
